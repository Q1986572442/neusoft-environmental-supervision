<template>
  <div class="swiss-dashboard">
    ...
    <section class="content-split-grid stretch-section form-split">
      ...
      <!-- NEPG网格员：指派给我的任务列表 -->
      <div class="form-panel glass-panel scrollable-card" v-if="currentRole === 'NEPG' && assignedTasks.length > 0" style="grid-column:1/-1;max-height:200px;margin-bottom:0">
        <div class="panel-header">
          <h3 class="panel-title"><el-icon><List /></el-icon> 指派给我的检测任务</h3>
          <span class="item-count">{{ assignedTasks.length }} 个待检测</span>
        </div>
        <div class="panel-body scroll-area" style="padding:8px 16px">
          <div style="display:flex;gap:12px;flex-wrap:wrap">
            <div v-for="task in assignedTasks" :key="task.id"
              @click="selectTask(task)"
              :style="{
                padding:'10px 16px', borderRadius:'12px', cursor:'pointer',
                background: form.feedbackId === task.id ? 'rgba(42,72,58,0.1)' : 'rgba(255,255,255,0.5)',
                border: form.feedbackId === task.id ? '2px solid #2A483A' : '1px solid rgba(0,0,0,0.08)',
                minWidth:'200px'
              }">
              <div style="font-size:12px;color:#86868B">#{{ task.id }} · AQI预估{{ task.estimatedAqiLevel }}级</div>
              <div style="font-size:13px;font-weight:600;color:#1C2421;margin-top:4px">{{ task.specificAddress || '地址未记录' }}</div>
            </div>
          </div>
        </div>
      </div>
...
import { submitAqi, getAqiPage, getMyAqiRecords } from '@/api/aqi'
import { getFeedbackPage } from '@/api/feedback'
import { ElMessage } from 'element-plus'
import {
  Back, Upload, Loading, Odometer,
  Compass, Link, Avatar, Connection, Document, List
...
async function fetchAssignedTasks() {
  if (currentRole.value !== 'NEPG') return
  tasksLoading.value = true
  try {
    const res = await getFeedbackPage(1, 50, 'ASSIGNED', currentUserId.value)
    assignedTasks.value = res.data || []
  } catch (e) {} finally { tasksLoading.value = false }
}

/** 选择一个指派任务，自动填入表单 */
function selectTask(task) {
  form.value.feedbackId = task.id
  form.value.inspectorId = currentUserId.value
  ElMessage.success(`已选择案件 #${task.id}`)
}
